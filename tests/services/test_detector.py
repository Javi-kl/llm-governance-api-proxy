...
from app.services.detector import Detection, analyze
...
def test_given_clean_text_then_no_detections():
    assert analyze("La capital de Francia es París.") == []
...
def test_given_valid_dni_then_detects():
    detections = analyze("Mi DNI es 12345678Z")

    assert len(detections) == 1
    assert detections[0] == Detection(
        category=SensitiveCategory.IDENTIFICATION,
        pattern_name="DNI",
        match="12345678Z",
        start=10,
        end=19,
...
def test_given_dni_in_mid_text_then_detects():
    detections = analyze("Titular: 12345678Z, fecha: ayer")
...
def test_given_dni_with_invalid_letter_then_no_detection():
    ...
    assert analyze("DNI 12345678I") == []


def test_given_eight_digits_without_letter_then_no_detection():
    assert analyze("Número 12345678 solo") == []
...
def test_given_valid_email_then_detects():
    detections = analyze("Mi email es javier@example.com")
...
def test_given_email_with_subdomain_then_detects():
    detections = analyze("Contacto: user@mail.example.org")
...
def test_given_email_without_tld_then_no_detection():
    ...
    assert analyze("admin@localhost") == []
...
def test_given_dni_and_email_then_both_detected():
    detections = analyze("DNI 12345678Z, email j@x.com")

    assert len(detections) == 2
    categories = {d.category for d in detections}
    assert categories == {SensitiveCategory.IDENTIFICATION, SensitiveCategory.CONTACT}


def test_given_two_emails_then_both_detected():
    detections = analyze("a@b.com y c@d.es")

    assert len(detections) == 2
    assert all(d.category == SensitiveCategory.CONTACT for d in detections)
...
def test_given_spanish_mobile_then_detects_contact():
    detections = analyze("Llámame al 612345678")
...
def test_given_valid_spanish_iban_then_detects_financial():
    detections = analyze("Cuenta ES9121000418450200051332")
...
def test_given_iban_with_spaces_then_detects():
    ...
    detections = analyze("ES91 2100 0418 4502 0005 1332")
...
def test_given_fake_iban_then_no_detection():
    ...
    assert analyze("ES0012345678901234567890") == []


# ── Ciclo 7: Tarjeta de crédito ────────────────────────────


def test_given_valid_credit_card_then_detects_financial():
    detections = analyze("Pago con 4111111111111111")
...
def test_given_credit_card_with_spaces_then_detects():
    ...
    detections = analyze("4111 1111 1111 1111")

    assert len(detections) == 1
    assert detections[0].pattern_name == "card"
...
def test_given_cp_with_prefix_then_detects():
    detections = analyze("CP 28001 Madrid")
...
def test_given_cp_with_codigo_postal_then_detects():
    detections = analyze("código postal: 08001 Barcelona")
...
def test_given_5_digit_amount_then_no_detection():
    ...
    assert analyze("total: 23456 euros") == []


def test_given_5_digit_invoice_then_no_detection():
    assert analyze("factura nº 28001") == []


def test_given_5_digits_in_text_then_no_detection():
    assert analyze("Madrid 28001 capital") == []
...
def test_given_valid_nie_then_detects():
    detections = analyze("NIE: X1234567L")
...
def test_given_nie_with_invalid_letter_then_no_detection():
    ...
    assert analyze("NIE: X1234567I") == []


def test_given_valid_cif_then_detects():
    detections = analyze("CIF: B12345678")
...
def test_given_match_at_text_start_then_detects():
    ...
    detections = analyze("612345678 llámame")
...
def test_given_match_at_text_end_then_detects():
    detections = analyze("Mi número 612345678")
...
def test_given_iban_does_not_trigger_other_patterns():
    ...
    detections = analyze("ES9121000418450200051332")
